...
void DRAW(void)
{
	if (player_ship_arrangement) //инструкци€
	{   
		P_Draw();
		printf("\n\n\n");
...
int get_num_of_ship(int x, int y) {
	for (int i = 0; i < 20; i++) {
...
bool strike_is_correct_B(int x, int y) {
	if ((P_field[x][y] != 'X' && P_field[x][y] != '*') && (y > -1 && y < 10) && (x > -1 && x < 10)) return(true);
...
bool finishing() {
	...
	int coord_strike1 = history_of_strike[count_of_history][0]; /// -1??
	int coord_strike2 = history_of_strike[count_of_history][1];
	...
	if (destroed_deck_B == deck_of_ship) {
		...
		int num = get_num_of_ship(coord_strike1, coord_strike2);
		P_list *temp;
		temp = p_root_list;
		while (num > 0) {
			temp = temp->p_next;
		...
		while (next_strike < 4) {// 6, 3, 12, 9  часов соответственно
			...
			if (coord_strike1 == 4 && coord_strike2 == 3) {
				int z = 0;
			}
			if (strike_is_correct_B(coord_strike1, coord_strike2)) {
				...
				for (int i = 0; i < 20; i++) {
					if (ship_coord_P[i][0] == coord_strike1) {
						if (ship_coord_P[i][1] == coord_strike2) {
							Life_P--;
							P_field[coord_strike1][coord_strike2] = 'X';
							Player_Field[coord_strike1][coord_strike2] = 'X';
							destroed_deck_B++;
				...
				//missed strike
				P_field[coord_strike1][coord_strike2] = '*';
...
*/
	...
	while (!strike_is_correct_B(coord1, coord2)) {
		coord1 = rand() % 10;
		coord2 = rand() % 10;
	...
	for (int i = 0; i < 20; i++) {
		if (ship_coord_P[i][0] == coord1) {
			if (ship_coord_P[i][1] == coord2) {

				if (P_field[coord1][coord2] != 'X' && P_field[coord1][coord2] != '*') {// ненужна проверка?
					Life_P--;
					P_field[coord1][coord2] = 'X';
					Player_Field[coord1][coord2] = 'X';
					destroed_deck_B++;
					deck_of_ship = check_type_of_ship(i);
					history_of_strike[count_of_history][0] = coord1;
					history_of_strike[count_of_history][1] = coord2;
					count_of_history++;
					finish_ship = true;
					sdv = 0;
				
					int num = get_num_of_ship(coord1, coord2);
					P_list *temp;
					temp = p_root_list;
					while (num > 0) {
						temp = temp->p_next;
		...
		if (ship_coord_B[i][0] == X_coord) {
			if (ship_coord_B[i][1] == Y_coord) {
				...
				if (P_T_field[Y_coord][X_coord] != 'X') {
					...
					//обрисовка убитого *
					int num = get_num_of_ship(X_coord, Y_coord);
					B_list *temp;
					temp = b_root_list;
					while (num > 0) {
						temp = temp->b_next;
